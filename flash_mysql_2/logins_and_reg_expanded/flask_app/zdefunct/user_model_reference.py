...
"""
    ...
    def validate(form):
        ...
        if len (form['first_name'])<3:
            flash("You must be some kinda joker, need more chars", "register")
            is_valid= False
        if len (form['last_name'])<3:
            flash("weak last name", "register")
            is_valid= False
        if len(form['email']) <3:
            flash ("ur email must be at least 3 chars", "register")
            is_valid=False
        if not EMAIL_REGEX.match(form['email']): 
            flash("Invalid email address!", "register")
            is_valid = False
        if not (re.search('[0-9]',form['password'])):
            flash("Please use one number", "register")
            is_valid = False
        if not (re.search('[A-Z]',form['password'])):
            flash("Please use one capital", "register")
            is_valid = False
        ...
        if len(form['password']) <3:
            flash ("r u even trying", "register")
            is_valid=False
        return is_valid
    @staticmethod
